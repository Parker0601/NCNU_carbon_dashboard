$(document).ready(function () {
  ...
  function statusToCalendarClass(s) {
    const st = displayStatus(s);
    if (st === '待審核')   return 'bg-warning border-warning text-dark';
    if (st === '審核完成') return 'bg-success border-success text-white';
    if (st === '已退回')   return 'bg-danger border-danger text-white';
    return '';
  }
  function badgeClass(s) {
    const st = displayStatus(s);
    if (st === '待審核')   return 'badge-warning';
    if (st === '審核完成') return 'badge-success';
    if (st === '已退回')   return 'badge-danger';
  ...
  async function fetchSchedule() {
    ...
    // 期待後端回傳：{ data: [ {id,title,start,end,ownerName,ownerId,status,desc,issueId,deviceName,rejectReason?} ... ] }
    const { data } = await apiFetch(ROUTES.list(from, to));
    currentSchedule = (data || []).map(e => ({
      id: e.id,
  ...
  async function approveTask(id) {
    ...
    // 本地同步
    const t = currentSchedule.find(x => String(x.id) === String(id));
  ...
  async function returnTask(id, reason, newEndIso) {
    await apiFetch(ROUTES.review(id), { method: 'PATCH', body: { action: 'return', reason, newEndTime: newEndIso } });
  ...
  function renderList(events) {
    ...
    events.forEach(e => {
      const st = displayStatus(e.status);
      let rowClass = '';
      if (st === '已退回') rowClass = 'table-danger';
      else if (st === '審核完成') rowClass = 'table-success';
      ...
      $tb.append(`
        <tr class="${rowClass}">
          ...
          <td><span class="badge ${badgeClass(e.status)}">${st}</span></td>
          <td>
